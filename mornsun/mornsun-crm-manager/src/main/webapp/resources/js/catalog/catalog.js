...
function reloadTree() {
	...
	commPostRequest("catalog/list", {}, function(data) {
		...
		//加载菜单树列表
		$('#searchCatalogParentIdTree').combotree("loadData", treeDataSearch);
		$('#addCatalogParentIdTree').combotree("loadData", treeDataAdd);
		$('#editCatalogParentIdTree').combotree("loadData", treeDataEdit);
...
function InitGrid(queryData) {
	$('#grid').datagrid({
		url:"catalog/pagination?sessionId="+sessionId,
		title: '分类列表',
		iconCls: 'icon-view',
//		height: 650,
...
function BindAddEvent() {
	$("#btnAddOK").unbind("click").click(function() {
		...
		commPostRequest("catalog/save", postData, function(){
...
function BindEditEvent() {
	$("#btnEditOK").unbind("click").click(function() {
		...
		commPostRequest("catalog/update", postData, function(data) {
