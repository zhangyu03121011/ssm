...
function InitGrid(queryData) {
	$('#grid').datagrid({
		url:"payscale/pagination?sessionId="+sessionId,
		title: '支付比例列表',
		iconCls: 'icon-view',
//		height: 650,
...
function BindAddEvent() {
	$("#btnAddOK").unbind("click").click(function() {
		...
		commPostRequest("payscale/save", postData, function(){
...
function BindViewInfo() {
	...
	if(data.scaleType==1){
		data.scaleType="平台";
	}else if(data.scaleType==2){
		data.scaleType="发布方";
	}else if(data.scaleType==3){
		data.scaleType="提问方";
	}else if(data.scaleType==4){
		data.scaleType="分享方";
	}else if(data.scaleType==5){
		data.scaleType="回答方";
	}
...
function BindEditEvent() {
	$("#btnEditOK").unbind("click").click(function() {
		...
		commPostRequest("payscale/update", postData, function(data) {
